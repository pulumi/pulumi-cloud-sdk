...
class EnvironmentSettings(PulumiAutoModelEncoder):
    """
    ...
    def copy_common_fields(self, source: Any, /) -> None:
        if isinstance(source, EnvironmentSettings):
    ...
    def deletion_protected(self, deletion_protected: 'bool'):
        if deletion_protected is None:
            raise ValueError("Invalid value for `deletion_protected`, must not be `None`")

        self._deletion_protected = deletion_protected

    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, EnvironmentSettings):
            return False
