	...
	if len(queryParams) > 0 {
		...
		for k, v := range queryParams {
			...
			case reflect.Slice, reflect.Array:
				// A repeated parameter is one key per value ("k=a&k=b"), which is
				// what the spec declares (OpenAPI's form/explode default), what the
				// TypeScript, Python, Java and .NET clients send, and what the
				// backend reads back with Query()[name]. Joining the values into a
				// single "k=a,b" reaches the handler as one uninterpretable value.
				for i := range elem.Len() {
					q.Add(k, fmt.Sprint(elem.Index(i).Interface()))
				}
			default:
...
func (p *CloudClient) invokeRaw(req *http.Request, headers []http.Header) (*http.Response, error) {
	...
	if resp.StatusCode >= 400 && resp.StatusCode <= 599 {
		...
		if parsed.Code == 0 {
			// Our error parsed as JSON, but it doesn't match the schema returned by API.
			// Use the HTTP status code and raw body.
...
func (p *CloudClient) ignoreClose(cr io.Closer) {
	if cr != nil {
		_ = cr.Close()
	}
}
