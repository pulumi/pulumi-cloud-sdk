.PHONY: build test publish \
	...
	build_python test_python publish_python \
	build_java test_java publish_java \
	build_dotnet test_dotnet publish_dotnet
...
test_nodejs:
	cd nodejs && npm test
publish_nodejs:
	cd nodejs && npm pkg set version=$(VERSION) && npm publish
...
publish_python:
	cd python && rm -rf dist *.egg-info && python3 -m venv .venv && \
		.venv/bin/pip install --upgrade pip build twine toml-cli && \
...
publish_dotnet:
	cd dotnet && PACKAGE_VERSION=$(VERSION) dotnet pack Pulumi.Cloud.Sdk.csproj -c Release -o nupkg && \
		dotnet nuget push "nupkg/*.nupkg" --api-key $(NUGET_PUBLISH_KEY) \
